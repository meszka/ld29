...
jaws.PixelMap.prototype.solidAtRect = function(rect, color) {
  var x = rect.x
  var y = rect.y

  for(; x < rect.right-1; x++)  if(this.solidAt(x, y)) return true;
  for(; y < rect.bottom-1; y++) if(this.solidAt(x, y)) return true;
  for(; x > rect.x; x--)      if(this.solidAt(x, y)) return true;
  for(; y > rect.y; y--)      if(this.solidAt(x, y)) return true;
...
jaws.PixelMap.prototype.solidAt = function(x, y) {
  var a = this.at(x, y);
  return a[3] != 0;
}
